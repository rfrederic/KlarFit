...
import { getRecommendation, TrainingGoal } from "@/lib/repsRecommendations";
...
export default function ProgramSectionCard({
  ...
  onOpenWorkout: (exerciseId: string) => void;
}) {
  return (
    <div className="rounded-lg border border-border bg-surface p-4 sm:p-5">
      ...
      <ul className="mt-3 flex flex-col gap-2">
        {section.exercises.map((exercise, i) => {
          const resolved = resolveProgramExercise(exercise, variant, rotationOptions);
          ...
          const restSeconds = libraryExercise
            ? getRecommendation(libraryExercise, rotationOptions.goal ?? "muscleGrowth", rotationOptions.experience)
                .restSeconds
            : undefined;
          ...
          return (
            <ExerciseRow
              ...
              onOpenWorkout={() => onOpenWorkout(exercise.id)}
            />
          );
        })}
